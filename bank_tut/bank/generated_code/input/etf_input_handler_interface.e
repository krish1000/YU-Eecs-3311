...
feature -- parsing
		...
		do
			...
			if cmd_name ~ "new" then 
				 if attached {ETF_STR_ARG} args[1] as id and then TRUE then 
					 create {ETF_NEW} Result.make ("new", [id.value], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 

			elseif cmd_name ~ "deposit" then 
				 if attached {ETF_STR_ARG} args[1] as id and then TRUE and then attached {ETF_INT_ARG} args[2] as amount and then TRUE then 
					 create {ETF_DEPOSIT} Result.make ("deposit", [id.value , amount.value], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 
			...
			elseif cmd_name ~ "withdraw" then 
				 if attached {ETF_STR_ARG} args[1] as id and then TRUE and then attached {ETF_INT_ARG} args[2] as amount and then TRUE then 
					 create {ETF_WITHDRAW} Result.make ("withdraw", [id.value , amount.value], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 

			elseif cmd_name ~ "transfer" then 
				 if attached {ETF_STR_ARG} args[1] as id1 and then TRUE and then attached {ETF_STR_ARG} args[2] as id2 and then TRUE and then attached {ETF_INT_ARG} args[3] as amount and then TRUE then 
					 create {ETF_TRANSFER} Result.make ("transfer", [id1.value , id2.value , amount.value], abstract_ui) 
				 else 
					 Result := dummy_cmd 
				 end 
			else 
	...
	do
		...
		loop
			...
			elseif cmd_name ~ "deposit" then 
				if NOT( ( args.count = 2 ) AND THEN attached {ETF_STR_ARG} args[1] as id and then TRUE and then attached {ETF_INT_ARG} args[2] as amount and then TRUE) then 
					if NOT Result.is_empty then
						Result.append ("%N")
					end
					Result.append (evt_out_str + " does not conform to declaration " +
			...
			elseif cmd_name ~ "transfer" then 
				if NOT( ( args.count = 3 ) AND THEN attached {ETF_STR_ARG} args[1] as id1 and then TRUE and then attached {ETF_STR_ARG} args[2] as id2 and then TRUE and then attached {ETF_INT_ARG} args[3] as amount and then TRUE) then 
					if NOT Result.is_empty then
						Result.append ("%N")
					end
					Result.append (evt_out_str + " does not conform to declaration " +
