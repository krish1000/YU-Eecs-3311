note
	description: "Summary description for {DBT_ITERATOR2}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"
...
feature
	make
		do
			reset_as_linear_db
			add_boolean_case(agent dbt_iterator2)
		end

feature
	dbt_iterator2: BOOLEAN
		local
			l_list: LIST[TUPLE[INTEGER, STRING]]
			l_answer_list: ARRAY[STRING]
	 	do
	 		comment ("dbt_iterator2: LINEAR_DB, int_str, go through 1 ~ 10, `a` ~ `j`")
	 		reset_as_linear_db
