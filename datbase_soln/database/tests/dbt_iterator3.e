note
	description: "Summary description for {DBT_ITERATOR3}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"
...
feature
	make
		do
			reset_as_linear_db
			add_boolean_case(agent dbt_iterator3)
		end

feature
	dbt_iterator3: BOOLEAN
		local
			l_list: LIST[TUPLE[STRING, STRING]]
			l_answer_list: ARRAY[STRING]
	 	do
	 		comment ("dbt_iterator3: LINEAR_DB, str_str, go through `a` ~ `j`")
	 		reset_as_linear_db
